(() => {
  // ../gym/gym/public/js/gym.bundle.js
  frappe.ui.form.ControlButton.prototype.make_input = function() {
    ...
    if (this.doctype == "Sales Order Item")
      ...
      btn_type = this.df.primary ? "btn-primary" : "btn-default";
    const btn_size = this.df.btn_size ? `btn-${this.df.btn_size}` : "btn-xs";
    this.$input = $(`<button class="btn ${btn_size} ${btn_type}">`).prependTo(me.input_area).on("click", function() {
      me.onclick();
    });
